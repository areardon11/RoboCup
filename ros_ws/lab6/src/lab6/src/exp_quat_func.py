...
def quaternion_to_exp(rot):
    ...
    theta - a scalar
    """
    theta = 2*math.acos(rot[-1])
    if theta == 0:
        omega = np.array([0,0,0])
    else:
        omega = rot[0:3] / math.sin(.5*theta)
    return (omega, theta)
...
def find_v(omega, theta, trans):
    ...
    A = np.dot((np.eye(3) - kfs.rotation_3d(omega, theta)), kfs.skew_3d(omega)) + np.dot(convert_1d_to_nd(omega), convert_1d_to_nd(omega).T)*theta
